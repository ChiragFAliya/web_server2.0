...
async fn update_task(app_state: web::Data<AppState>, task: web::Path<Task>) -> impl Responder {
    let mut db: std::sync::MutexGuard<'_, Database> = app_state.db.lock().unwrap();
...
async fn login(app_state: web::Data<AppState>, user: web::Json<User>) -> impl Responder {
    ...
    match db.get_user_by_name(&user.username) {
        Some(stored_user ) if stored_user.password ==  user.password => {
            HttpResponse::Ok().body("login sucessful")
        },  
        _ => HttpResponse::BadRequest().body("invalid username ot password! ")
    }
}
